...
bool ObjectManager::update() {
	...
	for (size_t i{}; i < num_obj; i++) {
		...
		if (obj) {
			obj->update();
			if (obj->getToDestruct()) destroyObject(obj->index);
		}
	}
	return true;
}
...
void ObjectManager::restartRoom() {
	...
	for (size_t i{}; i < num_objs_at_goto; i++) {
		...
		size_t* world_obj_index = room->getObjectIndexFromArray(0);
		if (world_obj_index == nullptr) break;
		if (obj_array[*world_obj_index] == nullptr) break;
		if (!obj_array[*world_obj_index]->getPersistent()) {
			room->removeObject(world_obj_index);
			destroyObject(*world_obj_index);
		}
...
Object::Object(ObjectManager* objMan, Room* room, double X, double Y) : spr{}, hasSpr{}, x{ X }, y{ Y }, objMan{ objMan }, persistent{}, room{ room }, toDestruct{}, direction{}, col_mask{} {
...
struct Game : Object {
	...
	void update() override {
		...
		room = objMan->roomMan->getRoomPointer(objMan->roomMan->getCurrentRoom());
...
GameManager::GameManager(ObjectManager* objMan) {
	...
	objMan->roomMan->addRoom(Utils::getStringFromFile("rooms/game.txt"),
		[objMan](Room* room) {
			...
			objMan->createObject(new Asteroid(objMan, room, 400, 200, "largeAsteroid"));
		}
	);
